...
export const authService = {
  ...
  async login({
    ...
    AuthResponse | { requires2fa: boolean; email: string }
  > {
    const response = await api.post<
      TokenPair & { requires2fa?: boolean; email?: string }
    >(AUTH_ENDPOINTS.LOGIN, { email, password })

    // Si requiere 2FA, devolvemos la respuesta especial
  ...
  async verify2fa(email: string, code: string): Promise<TokenPair> {
    const response = await api.post<TokenPair>(
      `${AUTH_ENDPOINTS.LOGIN.replace('login', 'verify-2fa')}`,
      { email, code }
    )
  ...
  async register(userData: {
    ...
    const response = await api.post<TokenPair>(
      USERS_ENDPOINTS.REGISTER_STAFF,
      userData
    )
    let flag = false
